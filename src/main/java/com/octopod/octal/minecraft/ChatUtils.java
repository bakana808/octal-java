package com.octopod.octal.minecraft;
...
public class ChatUtils {
	...
	public enum Color {
		...
		private static Map<Character, Color> map = new HashMap<>();
	...
	public enum Format {
		...
		Character character = null;
		private static Map<Character, Format> map = new HashMap<>();
			...
			if(nextIsColorCode) {
				...
				if(color != null && format == null) { //This is a color
					...
					if(!text.toString().equals("")) {
						cb.append(text.toString()).color(lastColor).format(formats.toArray(new Format[formats.size()]));
					}
					//Reset variables
